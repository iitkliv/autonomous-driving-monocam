...
def extract(i):
	...
	y_.append([float(logs[i][3]), float(logs[i][4])])

	img_path = logs[i][1]
	img_path = data_folder+'IMG'+(img_path.split('IMG')[1]).strip()
	img = plt.imread(img_path)
	X_.append(image_preprocessing(img))
	y_.append([float(logs[i][3]) + delta_, float(logs[i][4])])
...
if __name__ == '__main__':
	...
	with open("model_throttle.json", "w") as json_file:
		json_file.write(model_json)

	model.save_weights("model_throttle.h5")
	print("Model Saved.")
